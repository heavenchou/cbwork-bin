...
open OUT, ">chk_epub_database.txt" or die "open chk_menu.txt error";


do1file("epub_database.txt");
...
sub do1file
{
	...
	while (<IN>)
	{
		...
		#T01,T01n0001,T0001,長阿含經,(22卷),後秦 佛陀耶舍共竺佛念譯
		($vol, $id, $number, $name, $juan, $author) = split(/\s*,\s*/,$_);
