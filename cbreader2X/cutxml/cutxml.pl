...
sub do1file 
{
	...
	# 各卷 PTS 開頭的處理法:
	# 每一卷最後一個 PTS 都要記錄下來, 例如 N01n0001.xml 第一卷最後的 PTS 頁碼是 <ref cRef="PTS.Vin.3.109"></ref>
	# 將此記錄在 $this_juan_last_pts[1] = '<ref cRef="PTS.Vin.3.109"></ref>';
	# 因此在下一卷一開始要加上 <ref cRef="PTS.Vin.3.109" type="PTS_hide"></ref> , 其中加上 type="PTS_hide" 是為了區別一般要呈現的標記.
...
sub get_juans
{
	...
	# <milestone> 已經移到 <pb><lb> 之前了, 所以此段不用了
=begin
...
{       
	...
	if ($el eq "ref")
	{
		...
		for my $attr ($map->getValues) 
		{
			if($attr->getName eq "cRef" && substr($attr->getValue, 0,4) eq "PTS.")
			{
...
sub add_tag_2_juan()
{
	for(my $i=1; $i<= $total_juannum; $i++)
	{
		# 把 <ref cRef="PTS.Vin.3.109"></ref> 換成 <ref cRef="PTS.Vin.3.109" type="PTS_hide"></ref>
		$this_juan_last_pts[$i-1] =~ s/><\/ref>/ type="PTS_hide"><\/ref>/;	
		# 把 <ref cRef="PTS.Vin.3.109"/> 換成 <ref cRef="PTS.Vin.3.109" type="PTS_hide"></ref>
