...
if($inputFile eq "" and $vol =~ /^(([TXJHWIABCFGKLMNPQSU]|(ZS)|(ZW)|(SL))\d*)n.*?\.xml$/)
{
	$inputFile = $vol;
	$vol = $1;
}
...
sub get_real_juan_num()
{
	...
	# SL,03,   ,0005, 1,華嚴一乘十玄門修學記（上）                    ,【智諭老和尚指導】
	# SL,04,   ,0005, 1,華嚴一乘十玄門修學記（中）                    ,【智諭老和尚指導】
	# SL,05,   ,0005, 1,華嚴一乘十玄門修學記（下）                    ,【智諭老和尚指導】
	if($file eq "SL04n0005.xml")
	{
		$ii = "002";
	}
	if($file eq "SL05n0005.xml")
	{
		$ii = "003";
	}
	# SL,17,   ,0022, 2,維摩詰所說經講記（上）                        ,【智諭老和尚講述】
	# SL,18,   ,0022, 2,維摩詰所說經講記（下）                        ,【智諭老和尚講述】
	if($file eq "SL18n0022.xml")
	{
		$ii = sprintf("%03d",$i+2);
	}
	# SL,19,   ,0023, 1,大方廣圓覺修多羅了義經修學記（上）            ,【智諭老和尚指導】
	# SL,20,   ,0023, 1,大方廣圓覺修多羅了義經修學記（下）            ,【智諭老和尚指導】
	if($file eq "SL20n0023.xml")
	{
		$ii = "002";
	}
	# SL,36,   ,0047, 1,佛七講話（一）                                ,【智諭老和尚講述】
	# SL,37,   ,0047, 2,佛七講話（二、三）                            ,【智諭老和尚講述】
	# SL,38,   ,0047, 2,佛七講話（四、五）                            ,【智諭老和尚講述】
	# SL,39,   ,0047, 3,佛七講話（六、七、八）                        ,【智諭老和尚講述】
	if($file eq "SL37n0047.xml")
	{
		$ii = sprintf("%03d",$i+1);
	}
	if($file eq "SL38n0047.xml")
	{
		$ii = sprintf("%03d",$i+3);
	}
	if($file eq "SL39n0047.xml")
	{
		$ii = sprintf("%03d",$i+5);
	}
