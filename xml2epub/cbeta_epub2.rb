...
require_relative 'unicode_service'
...
<body>
  ...
  def e_graphic(e)
    ...
    if $seeland_canon.include? $canon
      src = File.join(@settings[:figures_seeland], url)
    else
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  ...
  def prepare_oebps
    ...
    if @settings[:front_page]
      if %w(DA ZY HM).include? @canon
        @settings[:front_page].sub!('readme.xhtml','readme_'+@canon+'.xhtml')
      end
    ...
    if @settings[:back_page]
      if %w(DA ZY HM).include? @canon
        src = @settings[:back_page].sub('donate.xhtml','donate_seeland.xhtml')
        copy_oebps_file(src, 'back.xhtml')
      else
