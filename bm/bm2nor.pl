...
sub get_source()
{
    ...
    for($i = 0; $i<=$#all_source; $i++)
    {
		...
		#S      SL0001_01_p0017 V1.0   2013/08/19    1  修大方廣佛華嚴法界觀門直解              【智諭老和尚撰述】
...
=cut
	...
	while (/^(.*?)(${tagword})/)
	{
		...
		if($thistag =~ /<\/[wouFTiPp]>/)	# 不管 </w> , </e>, </F>, </T>, </i>, </P>, </p>
		{
			s/<\/[wouFTiPp]>//;
			next;
		}
		...
		if($thistag =~ /<quote .*?>/)	# 西蓮出處連結, 例如 : SL01n0001_p0020a02_##...佛於經中說，<quote T09n0262_p0007c07-09>舍利弗！汝等當一心...</quote>
		{
			s/<quote .*?>//;
			next;
		}
...
=cut
	...
	while(/^$pattern/)
	{
		...
		else
		{
			...
			if($word =~ /^\[[^\[ \da-z]{2,}?\]$/)	# 判斷是不是組字式 , [ ] 中不能有 [ , 數字, 空白, 英文字 (因為經文中有 [ka] 這一類的
			{
				# 找出其 CB 碼
