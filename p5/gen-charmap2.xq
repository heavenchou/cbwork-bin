...
{
for $g in distinct-values(collection($coll)//xsl:output-character/@string)
order by $g
return
if (contains($g, '#CB')) then
 let $s := xs:integer(substring(substring-after($g, '#CB'), 1, 5))
 return
 <xsl:output-character character="{codepoints-to-string($s + 983040)}" string="{$g}"/>
 ...
 let $s := loc:hexval( substring(substring-after($g, '#SD'), 2, 4))
 return
 <xsl:output-character  character="{codepoints-to-string($s + 1024000 ) }" string="{$g}"/>
